# Marker UI â€” Extreme Deep Code Review and Research-Backed Remediation Plan
...
4. **Audio UI has many â€œprofessionalâ€ controls, but the backend only has a local faster-whisper adapter.** Cloud STT provider records and capability rows exist, but provider adapters are not shipped. Enhancement, contradiction detection, fusion, and benchmark controls largely do not alter output.
5. **Privacy is still leaky in the frontend.** `ReactMarkdown` auto-renders image URLs through a normal `<img src=...>` path. A converted Markdown document can cause the userâ€™s browser to request external image URLs.
...
- **Strategic gap:** The code may not be â€œwrong,â€ but the product promise, docs, UI, and implementation do not yet match a production-grade standard.
...
> Build custom only when Marker UI can produce a materially better outcome than existing open-source or provider-native solutions. â€œMaterially betterâ€ means better privacy, better provenance, better user workflow, better engine routing, or better quality measurement â€” not simply â€œwe can write our own version.â€

This implies:

- Do **not** build a custom semantic chunker from scratch before evaluating Doclingâ€™s `HybridChunker` and hierarchical chunkers.
...
Custom work is justified in these areas because it is Marker UIâ€™s product value:
...
### 2.3 â€œExcellent featureâ€ definition

A feature should not ship as â€œpresentâ€ until all of these are true:
...
## F-001 â€” Native converters can return Markdown while the system labels it JSON/HTML/chunks
...
- `backend/app/services/format_store.py` defines `SUPPORTED_FORMATS = ("markdown", "json", "html", "chunks")` at lines 27â€“30 and uses this in cache parsing/merging.
- `backend/app/routes/convert.py` accepts `output_format` and `output_formats` with the same four options in the upload route.
- `frontend/src/lib/api.ts` defines `OutputFormat = 'markdown' | 'json' | 'html' | 'chunks'` at lines 7â€“8.
...
The same risk applies to HTML/chunks. The chunk case is especially misleading because â€œchunksâ€ implies RAG chunks, while current non-PDF paths produce normal Markdown.

### Why this is serious

This breaks downstream automation. A user, MCP client, or RAG pipeline may trust the file extension/content type and parse the result as JSON. It also creates long-term compatibility debt: once clients depend on â€œJSONâ€ containing Markdown, fixing it later becomes breaking.
...
## F-002 â€” Runtime fallback to `marker_pdf` is unsafe and can fail on unsupported formats
...
Replace â€œfallback to marker_pdf for all non-marker failuresâ€ with a **typed fallback policy**.
...
## F-003 â€” Engine override can mislabel resource requirements and route Marker work to CPU thread pool
...
2. If the user chooses `marker_pdf`, the plan must always reflect Markerâ€™s own requirements, not the native entryâ€™s requirements.
...
## F-004 â€” â€œChunksâ€ means offset paging, not semantic/RAG chunking
...
#### Tier 2 â€” Markdown structural chunker for Markdown-only outputs
...
## F-005 â€” Native converter implementation is too hand-written for the quality target
...
Some of this is good as a lightweight deterministic fallback. It is not enough as the main â€œproduction-grade, layout-aware, search-optimizedâ€ route for every non-PDF format.
...
## F-006 â€” Audio provider layer exists, but only local faster-whisper actually ships
...
## F-007 â€” Audio enhancement, correction, fusion, contradiction, and benchmark controls are mostly UI/metadata, not implemented features
...
#### Tier B â€” Deterministic extractive notes
...
This can remain custom because it is simple, private, and provenance-preserving. But it must be described as â€œextractive deterministic notes,â€ not â€œcorrected transcript.â€

#### Tier C â€” LLM enhancement/correction
...
- Contradiction fixture with â€œbudget is approvedâ€ and â€œbudget is not approvedâ€ produces a contradiction section with segment refs.
...
- â€œCorrectedâ€/â€œenhancedâ€ claims are validated and auditable.
- Raw transcript is always preserved.

---

## F-008 â€” Frontend Markdown preview auto-loads remote images
...
4. Add a user setting:
   - â€œAuto-load external images in converted Markdownâ€ default false.
...
- Click â€œload onceâ€ and assert only that specific URL loads.
...
## F-009 â€” REST cancel/delete lifecycle is destructive and inconsistent with MCP
...
So â€œcancelâ€ does not mean â€œmark job cancelled and preserve history.â€ It means â€œremove DB row, optionally keep files.â€
...
- Frontend buttons:
  - â€œCancelâ€ while pending/processing.
  - â€œDeleteâ€ in history.
  - â€œRemove filesâ€ secondary option.
...
## F-010 â€” Format support and option schemas are duplicated across REST, CLI, MCP, frontend, and docs
...
- Audio output modes: backend accepts `interview_qna` and `action_decision_log` in REST, while `agent_contract.py` still limits `AudioOutputMode` to `transcript`, `enhanced`, `notes`, `meeting_notes`, `lecture_notes` at line 25. `agent_api.build_conversion_config` also only accepts those older modes at lines 173â€“174.
...
## F-011 â€” Output writer and downloader disagree on extension/content semantics
...
## F-012 â€” Audio/video/ZIP metadata can become very large in DB rows
...
## F-013 â€” Source URL SSRF protection is good but still has DNS TOCTOU risk
...
## F-014 â€” ZIP conversion needs stronger archive-bomb and output-asset behavior
...
## F-015 â€” Video conversion is a prototype, not a production multimodal pipeline
...
## F-016 â€” MCP surface is promising but not enterprise-complete
...
### Phase 0 â€” Stop wrong outputs and unsafe fallbacks
...
### Phase 1 â€” Introduce structured conversion backends
...
### Phase 2 â€” Real semantic chunks
...
### Phase 3 â€” Audio provider and enhancement maturity
...
### Phase 4 â€” MCP/CLI enterprise hardening
...
### Phase 5 â€” Video and archive quality
...
## 7. Definition of â€œdoneâ€ per major solution
...
The most important shift is to stop thinking of â€œconversionâ€ as one text string. Treat every conversion as a typed artifact with renderings, assets, chunks, metadata, and provenance. Once that foundation exists, the surrounding features â€” chunking, MCP resources, audio notes, downloads, previews, audits, and benchmarks â€” become simpler and more reliable.

---

## Appendix A â€” Code evidence index

- `backend/app/conversion/router.py:245-264` â€” engine override logic can mix marker engine with native metadata/resource requirements.
- `backend/app/conversion/registry.py:76-115` â€” registry selects by engine name and does not enforce `accepts(...)` at lookup.
- `backend/app/services/conversion_service.py:322-350` â€” generic runtime fallback from any non-marker engine to marker_pdf.
- `backend/app/conversion/converters/marker_pdf.py:37-52` â€” MarkerPdfConverter supports only PDF/images/EPUB.
- `backend/app/conversion/converters/text_data.py:95-123` â€” text/CSV/JSON/JSONL converter returns Markdown extension.
- `backend/app/conversion/converters/html.py:50-54` â€” HTML converter returns Markdown extension.
- `backend/app/conversion/converters/spreadsheet.py:71-75`, `112-116` â€” spreadsheet converter returns Markdown extension.
- `backend/app/conversion/converters/office_docx.py:182-187` â€” DOCX converter returns Markdown extension.
- `backend/app/conversion/converters/office_pptx.py:299-304` â€” PPTX converter returns Markdown extension.
- `backend/app/conversion/converters/archive.py:126-130` â€” archive converter returns Markdown extension.
- `backend/app/conversion/converters/audio.py:127-164` â€” audio converter returns Markdown and embeds transcript metadata.
- `backend/app/conversion/converters/video.py:95-122` â€” video converter returns Markdown and embeds transcript/frame metadata.
- `backend/app/services/format_store.py:27-30` â€” global format list includes markdown/json/html/chunks.
- `backend/app/services/task_manager.py:80-103` â€” finalizer cache can store primary text under requested primary format.
- `backend/app/services/output_writer.py:137-146` â€” output writer maps chunks to json.
- `backend/app/routes/convert.py:122-127` in download route â€” downloader maps chunks to txt.
- `frontend/src/components/features/OutputViewer.tsx:218-246` â€” Markdown image renderer directly emits `<img src={src}>`.
- `backend/app/audio/providers/registry.py:36-49`, `57-104` â€” only local faster-whisper adapter ships; cloud providers deferred.
- `backend/app/audio/providers/capabilities.py:177-227` â€” capability matrix advertises providers and availability flag.
- `backend/app/audio/pipeline.py:219-271` â€” enhanced audio render is deterministic/extractive and does not implement separate structural modes.
- `frontend/src/components/features/audio/AudioAdvancedSettings.tsx:106-258` â€” UI exposes advanced audio controls that backend largely does not honor.
- `backend/app/mcp_server.py:250-289` â€” `marker_read_output` and `marker_read_output_chunk` both call offset-based `read_output`.
- `backend/app/agent_api.py:119-169` â€” output chunk reading is character offset paging.
- `backend/app/routes/convert.py:265-300` â€” REST delete cancels and deletes in one operation.
- `backend/app/agent_api.py:224-238` â€” agent delete helper deletes DB row even when `delete_files=False`.
- `backend/app/services/safe_url_fetcher.py:157-179` â€” safe URL hostname/IP validation before fetch.

---

## Appendix B â€” External source notes
