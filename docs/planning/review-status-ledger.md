...
findings after checking the local repository. The original external plan files
were not available in `Downloads` during this audit, and the old ignored
`planning/` copies were removed from tracked source. Keep future summaries here
using generic paths only.
