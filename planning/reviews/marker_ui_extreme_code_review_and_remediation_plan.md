...
The review intentionally goes beyond the issues already reported by the coding agent. The strongest pattern is not “one missing feature,” but **architecture drift**: multiple independent registries, multiple contracts, aspirational UI controls, and native converters that produce Markdown while the UI/API advertise structured multi-format behavior.
...
2. **Chunking is not semantic/RAG chunking except where Marker’s own chunk renderer is used.** `marker_read_output_chunk` is just offset paging.
...
### MUI-001 — Non-PDF output formats are falsely advertised and can produce mislabeled files
...
The dangerous part is the finalization helper: when a native converter returns one Markdown envelope but the job’s primary `output_format` is `json`, `_formats_payload_for_finalize()` still stores the Markdown text under the key `json`. The download route then maps `json` to `.json` and `application/json` even if the content is a Markdown table or fenced code block.
...
### MUI-002 — Runtime fallback to `marker_pdf` is unsafe and can route unsupported files into Marker
...
### MUI-003 — Job submission can race database commit and cause skipped finalization
...
### MUI-004 — Cancel/delete lifecycle is inconsistent and running thread cancellation is not reliable
...
### MUI-005 — Markdown preview can leak remote image requests
...
- Render blocked images as a privacy notice with a “copy URL” action, not an auto-fetch.
...
### MUI-006 — RAG/semantic chunking is absent or only available on the Marker stock chunk renderer path
...
Marker’s own `chunks` renderer can work for Marker-backed PDF/image paths, but that does not solve native converters or agent reading.
...
### MUI-007 — Audio provider registry advertises deferred providers but only local faster-whisper actually ships
...
- UI displays deferred providers in a “coming soon” read-only comparison table, not in the provider dropdown.
...
### MUI-008 — Audio advanced controls are mostly no-op or deterministic shells
...
- Turning on “Improve Transcript Wording” changes output only through the enhancement layer and records an audit diff.
- Turning on “Contradiction Detection” produces a `contradictions` metadata block or the UI disables it.
- Turning on “Compare Providers” actually runs the selected comparison providers or is not available.
...
### MUI-009 — Unknown audio provider silently falls back to local faster-whisper
...
### MUI-010 — Video conversion is experimental but exposed as if it is a meaningful multimodal timeline
...
### MUI-011 — ZIP archive conversion loses child assets and is not yet a serious batch document artifact
...
- Manifest lists each child’s status, output, assets, and failure reason.
...
### MUI-012 — Native Office/spreadsheet converters are too shallow for the product standard
...
These are fine as first-pass deterministic converters, but they are not enough for “production-grade, layout-aware, search-optimized” native conversion.
...
This keeps custom code focused on orchestration, privacy, provenance, and UX — the project’s differentiator — not on reimplementing Office parsers.
...
### MUI-013 — Format and capability support is duplicated across too many places
...
### MUI-014 — Agent/MCP/REST contracts have real schema drift
...
### MUI-015 — Source URL SSRF protection is good but still has DNS/connect-time gaps
...
### MUI-016 — Database migrations are not production-grade
...
### MUI-017 — Backend type/lint posture is too weak for the project ambition
...
### MUI-018 — Image-understanding pipeline has powerful custom code with global side effects
...
### MUI-019 — Documentation and README overstate feature completeness
...
## 5. Architecture plan: “zero to one hundred” without overengineering
...
```text
                  ┌──────────────────────────┐
                  │ Capability Registry       │
                  │ formats/engines/options   │
                  └─────────────┬────────────┘
                                │
      REST / GUI / CLI / MCP ◄──┼──► Generated schemas/types/docs
                                │
                  ┌─────────────▼────────────┐
        ...
        │                       │                         │
┌───────▼───────┐     ┌─────────▼────────┐       ┌────────▼────────┐
│ Marker Backend │     │ Docling Backend   │       │ MarkItDown/Pandoc│
│ PDF/image/OCR  │     │ structured IR     │       │ markdown fallback│
└───────┬───────┘     └─────────┬────────┘       └────────┬────────┘
        │                       │                         │
        └──────────────┬────────┴──────────────┬──────────┘
                       ▼                       ▼
...
#### Phase 0 — Stabilize truthfulness and prevent bad outputs
...
#### Phase 1 — Canonical artifact and real chunking
...
#### Phase 3 — Native conversion excellence
...
#### Phase 4 — Job lifecycle, queue, observability, enterprise polish
...
## 9. What “successful implementation” means
